    ...
    it('can find using filter', async () => {
        ...
        expect(mockedAxios.post).toHaveBeenNthCalledWith(
            ...
            '/2.1/value',
            {
                ...
                return: '{value (attribute: ["this_type=energy"]) { meta{id type application version connection name_by_user} name permission description type measure_type period delta thresholds number string blob xml status state  { meta{id type application version connection name_by_user} data type timestamp }}}',
            },
            {
                params: {
                    expand: 1,
